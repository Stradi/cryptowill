<template>
  <div>
    ...
    <div class="p-5 rounded-3xl bg-gray-800 text-left">
      ...
      <div class="pt-2 pb-4">
        <label for="confirmAddress" class="font-medium">Address</label>
        <input v-model="address" name="confirmAddress" id="confirmAddress" type="text" :disabled="!this.$store.state.contract.isInitialized" placeholder="0x0000000000000000000000000000000000000000" @input="onAddressChange" :class="isAddressValid ? 'ring-yellow-500' : address == '' ? '' : 'ring-red-600'" class="ring-2 ring-transparent mt-1 p-2 text-black rounded-2xl w-full outline-none">
      </div>
      {{ this.errorMessage }}
