...
export default {
  ...
  methods: {
    ...
    async withdraw() {
      ...
      if(this.canWithdraw) {
        //Withdraw
        await this.$store.dispatch("contract/withdraw", { payerAddress: this.address });
      } else {
        //Check availability
        contract.checkWithdrawAvailability(this.$store.state.contract.instance, this.$store.state.web3.account, { payerAddress: this.address }).then(() => {
